...
void monteCarloCD_a(int argc, char *argv[]) {
    if (argc != 4) {
    ...
    int output_fd = open("RequisitoC_a.txt", O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
    if (output_fd == -1) {
    ...
    for (i = 0; i < num_children; i++) {
        ...
        if (pid == 0) {
            ...
            int n_bytes = writen(fds[1], msg, msg_len);
            close(fds[1]);
            exit(n_bytes);
        } else if (pid < 0) {
    ...
    for (i = 1; i <= num_children; i++) {
        wait(&status);
        read(fds[0], tampao, WEXITSTATUS(status));
        write(output_fd, tampao, WEXITSTATUS(status));
    ...
    int result_file = open("RequisitoC_a.txt", O_RDONLY);
    if (result_file == -1) {
        char error_msg[] = "Erro ao abrir o arquivo\n";
        write(STDERR_FILENO, error_msg, strlen(error_msg));
        exit(EXIT_FAILURE);
