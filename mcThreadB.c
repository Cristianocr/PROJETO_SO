...
void *worker_thread(void *args) {
    ...
    for (int j = start_point; j < end_point; j++) {
        ...
        pthread_mutex_lock(thread_args->mutex);
        thread_args->points_processed += 1;
...
/**
 * Funçao para cria barra de percentagem
 * @param args
 * @return
 */
...
void mcThreadB(int argc, char *argv[]) {
    if (argc != 4) {
    ...
    pthread_t threads[W], barra;

    pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;
    ThreadArgs thread_args = {polygon, random_points, MAX_POINTS, 0, M, W, &mutex, 0, 0};

    if (pthread_create(&barra, NULL, barra_thread, &thread_args) != 0) {
