...
void monteCarloE_b(int argc, char *argv[]) {
    if (argc != 4) {
    ...
    int output_fd = open("RequisitoE_b.txt", O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
    if (output_fd == -1) {
    ...
    for (i = 0; i < num_children; i++) {
        ...
        while (1) {
            ...
            if ((bytes_read = read(new_sockfd, buffer, sizeof(buffer))) < 0) {
    ...
    int result_file = open("RequisitoE_b.txt", O_RDONLY);
    if (result_file == -1) {
